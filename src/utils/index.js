...
export function toThousands(num = '') {
  ...
  const nums = Math.round(num * 10) / 10; // 保留一位小数且四舍五入
  const n = numFormat(nums);
  if (!n.includes('.')) {
    return n + '.0';
  }
  return n;
...
/*
  desc：百分比处理,  传入数字或字符串都可以，百分位保留1位小数，千分位处理
  params：
  return： string 返回字符串
  scope：全局使用
*/
...
//       if (top && top.location.origin) {
//         let woPrefix = top.location.origin;
//         window.location.href = woPrefix + '/wiseoper/w3Logout';
//       }
//     }
//   });
// };

// 未用，数字千分位处理  支持小数 传入数字或字符串
...
// 秒转时，保留一位小数，四舍五入
export function secondsToHours(num) {
  if (num === 0 || num === '0') {
    return '0.0';
  }
  if (!num || num === '-') {
  ...
  const nums = Math.round(hours * 10) / 10; // 保留一位小数且四舍五入
  const n = numFormat(nums);
  if (!n.includes('.')) {
    return n + '.0';
  }
